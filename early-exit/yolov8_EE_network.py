...
class YOLOv8n_EE(nn.Module):
    ...
    def __init__(self,nc , model_yaml):
        ...
        self.exit3 = Detect(nc=self.nc, ch=[512])
    ...
    def forward(self, x, exit_conf_threshold=0.25):
        ...
        if self.fast_inference_mode and pred0[0][:,4].max() > exit_conf_threshold:
        ...
        if self.fast_inference_mode and pred1[0][:,4].max() > exit_conf_threshold:
            print(f"Early Exit - Layer 1/Result {pred1}")
            return (pred1, '1')
        ...
        if self.fast_inference_mode and pred2[0][:,4].max() > exit_conf_threshold:
            print(f"Early Exit - Layer 1/Result {pred2}")
            return (pred2, '2')

        x7 = self.conv7(x6)                   # P5
        x8 = self.c2f8(x7)
        x9 = self.sppf9(x8)                   # enriched P5
        pred3 = self.exit3([x6])              # early-exit head 3

        if self.fast_inference_mode and pred3[0][:,4].max() > exit_conf_threshold:
            print(f"Early Exit - Layer 1/Result {pred3}")
            return (pred3, '3')
        ...
        x12= self.c2f12(f11)
        pred4 = self.exit4([x6])              # early-exit neck 1

        if self.fast_inference_mode and pred4[0][:,4].max() > exit_conf_threshold:
            print(f"Early Exit - Layer 1/Result {pred4}")
            return (pred4, '4')
