...
def print_and_save_results(model_path, test_dir, image_paths, imgsz, device, batch, map_50_95, map_50, 
    precision, recall, avg_ms, fps,total_s, results_txt, out_dir):
    """
    ...
    if not (precision != precision):  # not NaN
        lines.append(f"Precision (mean): {precision:.4f}")
    if not (recall != recall):  # not NaN
        lines.append(f"Recall (mean):    {recall:.4f}")

    lines.append("")
    lines.append("== == == == Inference Speed Metrics == == == ==")
